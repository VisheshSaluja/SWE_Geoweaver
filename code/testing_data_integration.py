...
def merge_all_gridmet_amsr_csv_into_one(gridmet_csv_folder, dem_all_csv, testing_all_csv, water_mask_csv, force=True):
    """
    ...
    Returns:
        None
    """
    if os.path.isfile(testing_all_csv) and not force:
        print(f"The file {testing_all_csv} exists. skipping.. ")
        return
...
if __name__ == "__main__":
    ...
    # Adjust test_year based on condition
    if current_year == test_year and current_month < 10:
      test_year -= 1
    ...
    merge_all_gridmet_amsr_csv_into_one(f"{work_dir}/testing_output/",
                                        ...
                                        f"{homedir}/water_mask/final_output/{test_year}_output.csv", force=True)

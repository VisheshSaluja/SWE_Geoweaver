...
parent_dir = "/home/chetana/modis_veg/"
...
tile_list = ["h08v04", "h08v05", "h09v04", "h09v05", "h10v04", 
  "h10v05", "h11v04", "h11v05", "h12v04", "h12v05",
...
my_env['GDAL_DATA'] = '/usr/share/gdal/2.2/'
...
def convert_hdf_to_geotiff(hdf_file, output_folder):
  ...
  target_subdataset_name = 'MODIS_Grid_16DAY_500m_VI:"500m 16 days NDVI"'
...
def merge_tifs(folder_path, target_date, output_file):
  ...
  print("len(tif_files) = ", len(tif_files))
  if len(tif_files) == 0:
    print(f"uh-oh, didn't find HDFs for date {target_date}")
    # print("generate a new csv file with empty values for each point")
    # gdal_command = ['/usr/bin/gdal_translate', '-b', '1', '-outsize', '100%', '100%', '-scale', '0', '255', '200', '200', f"{modis_day_wise}/vegetation_template.tif", output_file]
    ...
    print("Running ", " ".join(gdal_command))
    subprocess.run(gdal_command, env=my_env,)
...
def merge_tiles(date, hdf_files):
  ...
  try:
    subprocess.run(" ".join(merge_command), env=my_env,)
...
def download_tiles_and_merge(start_date, end_date):
  date_list = [
    ...
    for i in range(0, (end_date - start_date).days + 1, 16)
  ]
  for i in date_list:
    current_date = i.strftime("%Y-%m-%d")
    target_output_tif = f'{modis_day_wise}/{current_date}__vegetation.tif'
    ...
    if os.path.exists(target_output_tif):
        ...
        results = earthaccess.search_data(short_name="MOD13A1", 
                                          cloud_hosted=True, 
